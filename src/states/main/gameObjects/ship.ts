...
export default class Ship extends GameObject {
	...
	public static teleport: any;

	public accel = 35;
	public drag = 0.98;
	public speedLimit = 660;
	public size = 30;
	public lastModelRotation = 0;
	...
	public active = false;
	public dead = false;
	public inContact = false;
	public controlBlock = false;
	public sounds: any = {};
	public wrapperObj: Object3D;
	public shipMaterial: MeshBasicMaterial;
	public controlFlags: any;
	public shield: any;
	public state: any;
	public contrailL: any;
	public contrailR: any;
	public propulsion0ModelLeftMat: any;
	public propulsion0ModelRightMat: any;
	public propulsion1ModelLeft: any;
	public propulsion1ModelRight: any;
	public propulsion1ModelLeftMat: any;
	public propulsion1ModelRightMat: any;
	public propulsion2ModelLeft: any;
	public propulsion2ModelRight: any;
	public propulsion2ModelLeftMat: any;
	public propulsion2ModelRightMat: any;
	...
	public create(): void {
		this.wrapperObj = new Object3D();

		super.add(this.wrapperObj);
		...
		var shipModel = new Mesh(Game.engine.loader.resources["ship01model"].geometry, this.shipMaterial);
	...
	public initSounds(): void {
		...
		sound.setBuffer(Game.engine.loader.resources["shipEngine"].buffer);
		sound.setLoop(true);

		this.sounds.engine = sound;

		sound = new Audio(Game.engine.audioListener);
		sound.setBuffer(Game.engine.loader.resources["energyShield"].buffer);
		sound.setLoop(true);

		this.sounds.energyShield = sound;

		sound = new Audio(Game.engine.audioListener);
		sound.setBuffer(Game.engine.loader.resources["shipExplode"].buffer);
		...
		var energyDown = new Audio(Game.engine.audioListener);
		energyDown.setBuffer(Game.engine.loader.resources["energyDown"].buffer);

		this.sounds.energyDown = energyDown;
	}

	public spendEnergy(amount): void {
		if (Game.engine.sound && this.energy > CONFIG.ENERGY_MANAGEMENT.WARNING && this.energy - amount < CONFIG.ENERGY_MANAGEMENT.WARNING) {
	...
	public collide(amount = 0, collideVelocity = 0): void {
		if (this.dead === true) {
			return;
		}
		if (!amount) {
	...
	public beginContact(amount, collideVelocity) {
	...
	public dieStep2(): void {
		this.wrapperObj.visible = false;
		if (Game.engine.sound) {
			this.sounds.explode.play();
		}
	}

	public initShield(): void {
		var material = {
			color: 0xffffff,
			map: Game.engine.loader.resources["ship01shield01"].texture,
			transparent: true,
		...
		Game.engine.scene.scene.add(this.shield);
	}

	public addEnergy(amount): void {
		this.energy += amount;
	...
	public initContrail(): void {
		this.contrailL = new Contrail(Game.engine, Game.engine.loader.resources.contrailTexture.texture, 9);

		this.contrailL.position.x = -30;
		this.contrailL.position.z = 2;
		this.contrailL.position.y = -9;

		this.wrapperObj.add(this.contrailL);

		this.contrailR = new Contrail(Game.engine, Game.engine.loader.resources.contrailTexture.texture, 9);

		this.contrailR.position.x = -30;
		this.contrailR.position.z = 2;
		this.contrailR.position.y = 9;

		this.wrapperObj.add(this.contrailR);
	}

	private velocityNull: number[] = [0, 0];

	public updateContrail = function () {
		this.contrailL.update(this.body.position, this.body.angle, this.wrapperObj.children[0].rotation.x, this.active === true ? this.body.velocity : this.velocityNull);

		this.contrailR.update(this.body.position, this.body.angle, this.wrapperObj.children[0].rotation.x, this.active === true ? this.body.velocity : this.velocityNull);
	}

	public initPropulsion(shipModel): void {
		var materialOptions0 = {
			color: 0xffffff,
			map: Game.engine.loader.resources["ship01Propulsion03texture"].texture,
			transparent: true,
			blending: AdditiveBlending,
			opacity: 0
		};

		var propulsion0ModelLeft = new Mesh(Game.engine.loader.resources["ship01Propulsion01model"].geometry, new MeshBasicMaterial(materialOptions0));
		var propulsion0ModelRight = new Mesh(Game.engine.loader.resources["ship01Propulsion01model"].geometry, new MeshBasicMaterial(materialOptions0));
		...
		var materialOptions1 = {
			color: 0xffffff,
			map: Game.engine.loader.resources["ship01Propulsion01texture"].texture,
			transparent: true,
			blending: AdditiveBlending,
			opacity: 0
		};

		var propulsion1ModelLeft = new Mesh(Game.engine.loader.resources["ship01Propulsion02model"].geometry, new MeshBasicMaterial(materialOptions1));
		var propulsion1ModelRight = new Mesh(Game.engine.loader.resources["ship01Propulsion02model"].geometry, new MeshBasicMaterial(materialOptions1));
		...
		map1.wrapS = map1.wrapT = map1.wrapS = map1.wrapT = RepeatWrapping;
		map1.repeat.set(1, 1);
		map1.repeat.set(1, 1);
		...
		var materialOptions2 = {
			color: 0xffffff,
			map: Game.engine.loader.resources["ship01Propulsion02texture"].texture,
			transparent: true,
			blending: AdditiveBlending,
			opacity: 0
		};

		var propulsion2ModelLeft = new Mesh(Game.engine.loader.resources["ship01Propulsion02model"].geometry, new MeshBasicMaterial(materialOptions2));
		var propulsion2ModelRight = new Mesh(Game.engine.loader.resources["ship01Propulsion02model"].geometry, new MeshBasicMaterial(materialOptions2));
		...
		map2.wrapS = map2.wrapT = map2.wrapS = map2.wrapT = RepeatWrapping;
		map2.repeat.set(1, 1);
		map2.repeat.set(1, 1);
	...
	public update(delta: number): void {
		if (this.active === true) {
			if (this.dead === false) {
				...
				var shipModel = this.wrapperObj.children[0];
				...
				if (Math.abs(this.lastModelRotation - shipModel.rotation.x) > 0.001) {
					...
					if (a1 < 1.57) {
						this.propulsion0ModelRightMat.opacity = this.propulsion1ModelRightMat.opacity = this.propulsion2ModelRightMat.opacity = 0.6 + Util.Random.get();
						this.propulsion1ModelRight.scale.z = 0.04;
						this.propulsion2ModelRight.scale.z = 0.12;
						this.propulsion1ModelRightMat.map.offset.y -= 0.05;
						this.propulsion2ModelRightMat.map.offset.y -= 0.03;
					} else if (a1 > 1.58) {
						this.propulsion0ModelLeftMat.opacity = this.propulsion1ModelLeftMat.opacity = this.propulsion2ModelLeftMat.opacity = 0.6 + Util.Random.get();
						this.propulsion1ModelLeft.scale.z = 0.04;
						this.propulsion2ModelLeft.scale.z = 0.12;
				...
				if (this.controlMode === 1) {
					...
					if (this.controlFlags.mouseButton3 === true && this.energy > CONFIG.ENERGY_MANAGEMENT.MIN_VALUE) {
						...
						this.propulsion0ModelLeftMat.opacity = this.propulsion0ModelRightMat.opacity = this.propulsion1ModelLeftMat.opacity = this.propulsion1ModelRightMat.opacity = this.propulsion2ModelLeftMat.opacity = this.propulsion2ModelRightMat.opacity = 1; //0.5 + Random.get();

						this.propulsion1ModelLeft.scale.z = this.propulsion1ModelRight.scale.z = 0.13;

						this.propulsion2ModelLeft.scale.z = this.propulsion2ModelRight.scale.z = 0.3;
						...
						if (this.propulsion0ModelLeftMat.opacity > 0) {
							...
							this.propulsion1ModelLeft.scale.z = this.propulsion2ModelLeft.scale.z = 0.001;
						}
						...
						if (this.propulsion0ModelRightMat.opacity > 0) {
							...
							this.propulsion1ModelRight.scale.z = this.propulsion2ModelRight.scale.z = 0.001;
						}
				...
				//         this.energy > CONFIG.ENERGY_MANAGEMENT.MIN_VALUE &&
				//         Game.engine.time - this.controlFlags.mouseDownTime < this.shootBlockTimeout
				//     ) {
				...
				//                             this.controlFlags.mouseDownTime = Game.engine.time;
				//                             break;
				//                         }
				//                     }
				//                 } else {
