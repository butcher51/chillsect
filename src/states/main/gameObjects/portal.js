...
var Portal = function () {
    ...
    var mesh = new Mesh(this.engine.loader.resources['portalModel'].geometry, new MeshBasicMaterial({
        color: 0xffffff,
        transparent: true,
        map: this.engine.loader.resources['portalTexture'].texture
    }));
    ...
    var mechMat = new MeshBasicMaterial({
        color: 0xffffff,
        map: this.engine.loader.resources['portal2Texture'].texture
    });
    ...
    for (var i = 0; i < 10; i++) {
        var meshMech = new Mesh(this.engine.loader.resources['portalModel2'].geometry, mechMat);
    ...
    var texture = this.engine.loader.resources['particle02'].texture.clone();
...
Portal.prototype.size = 80;

Portal.prototype.out = function (gameObject) {
    ...
    t1.start(that.engine.time);

    setTimeout(function () {
        t2.start(that.engine.time);
    }, 1000);

    setTimeout(function () {
        t3.start(that.engine.time);
    }, 1500);
...
Portal.prototype.in = function (gameObject) {
    ...
    var cameraPosition = this.engine.scene.camera.position;
    ...
    var t1 = new TWEEN.Tween(coords)
        ...
        .onComplete(function () {
            ...
            setTimeout(function () {
                that.engine.world.removeBody(gameObject.body);
            }, 16);
...
Portal.prototype.update = function () {
    ...
    this.ledMat.opacity = Math.sin(this.engine.time * 4);
