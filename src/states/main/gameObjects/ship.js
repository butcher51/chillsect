...
class Ship extends GameObject {
	...
	shootBlockTimeout = 0.1;

	energy = CONFIG.ENERGY_MANAGEMENT.ENERGY_MAX_VALUE;
	plutonium = CONFIG.ENERGY_MANAGEMENT.PLUTONIUM_MAX_VALUE;
	...
	collide(amount, collideVelocity) {
		if (this.dead === true) {
			return;
		}
		if (!amount) {
	...
	beginContact(amount, collideVelocity) {
	...
	updateContrail() {
		this.contrailL.update(this.body.position, this.body.angle, this.warpperObj.children[0].rotation.x, this.active === true ? this.body.velocity : this.velocityNull);

		this.contrailR.update(this.body.position, this.body.angle, this.warpperObj.children[0].rotation.x, this.active === true ? this.body.velocity : this.velocityNull);
	}

	initPropulsion(shipModel) {
