...
export default class Preload extends State {
	...
	public start(params: any): void {
		...
		Game.engine.loader.loadLevel("levels/" + LevelManager.getLevelState().level + ".json", this.levelLoadedCallback.bind(this));
	}

	public levelLoadedCallback(levelData) {
		var assets = Util.mergeObjects(this.commonAssets, levelData.assets);

		Game.engine.loader.load(assets, this.loaded.bind(this));
	};
	...
	public loaded(): void {
		...
		startMission.onclick = () => {
			...
			Game.engine.states.start("main");
			Game.engine.initStats();
		};
