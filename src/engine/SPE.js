import {Math as _Math,Vector4,Points,ShaderMaterial,BufferGeometry,BufferAttribute,Color,Vector2,Texture,MeshLambertMaterial,Object3D,Raycaster,Plane,Vector3,RepeatWrapping,Mesh,AdditiveBlending,PointLight,SpotLight,MeshBasicMaterial,PlaneBufferGeometry} from 'three';

/* shader-particle-engine 1.0.5
 * 
 * (c) 2015 Luke Moody (http://www.github.com/squarefeet)
 *     Originally based on Lee Stemkoski's original work (https://github.com/stemkoski/stemkoski.github.com/blob/master/js/js/ParticleEngine.js).
 *
...
SPE.shaders = {
    ...
    fragment: [
        ...
        '   vec3 outgoingLight = vColor.xyz * rotatedTexture.xyz;',
 ...
 *
 * @property {Number} blending One of js's blending modes to apply to this group's `ShaderMaterial`.
 *
...
SPE.Group = function (options) {
   ...
   // this.textureFrames = utils.ensureInstanceOf(options.texture.frames, Vector2, new Vector2(1, 1));
    this.textureCrop = utils.ensureInstanceOf(options.texture.crop, Vector2, new Vector2(1, 1));
   ...
   // this.textureFrames.max(new Vector2(1, 1));
    ...
    this.hasPerspective = utils.ensureTypedArg(options.hasPerspective, types.BOOLEAN, false);
    ...
    this.fog = utils.ensureTypedArg(options.fog, types.BOOLEAN, false);
 ...
 * If there are no gameObjects in the pool, a new emitter will be
 ...
 * @param {EmitterOptions|Array} emitterOptions  An object, or array of gameObjects, describing the options to pass to each emitter.
 ...
 *                               Depending on the value of SPE.valueOverLifetimeLength, if arrays of Color instances are given, then the array will be interpolated to
