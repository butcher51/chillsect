...
var Controls = function (engine) {
    ...
    function onDocumentKeyDown(event) {
        ...
        if (event.keyCode === 37 || event.keyCode === 65) {//  jobb, d
            controlFlags.keyLeft = true;
        }
        if (event.keyCode === 39 || event.keyCode === 68) {// bal, a
            controlFlags.keyRight = true;
        }
        if (event.keyCode === 38 || event.keyCode === 87) {// előre, w
            controlFlags.keyUp = true;
        }
        if (event.keyCode === 40 || event.keyCode === 83) {// hátra. s
