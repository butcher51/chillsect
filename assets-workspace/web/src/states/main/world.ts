...
export default class World extends GameObject {
  ...
  shift: number;
  frameStep: number;
  frameFrekvency: number;
  distancLimitSquared: number;
  intMap: Int32Array;
  floatMap: Float32Array;
  slength: number;
  ...
  constructor() {
    ...
    var bufferData = Game.engine.loader.resources.level.data['buffers'];
    ...
    buffer = Game.engine.loader.resources['mapByteBuffer'].byteArray;
    ...
    console.log('Object count', '?TODO , byteLength?', buffer.length / this.shift / 4);
  }
  ...
  public update() {
    ...
    var k = 0,
      j = Number.parseInt('' + this.frameStep * this.slength); //TODO string parseint is slow?
    var jl = Number.parseInt('' + j + this.slength) + 1;
    for (; j < jl; j++) {
      i = j * this.shift;
      x = intMap[i] - sx;
      y = intMap[i + 1] - sy;
