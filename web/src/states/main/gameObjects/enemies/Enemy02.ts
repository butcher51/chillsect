...
export default class Enemy02 extends AbstractEnemy {
  ...
  constructor() {
    ...
    // var protoScene = Game.engine.loader.resources['Enemy02ModelAnim'].scene;

    if (!Enemy02.material) {
      Enemy02.material = new MeshBasicMaterial({
        color: 0xffffff,
        transparent: true,
        map: Game.engine.loader.resources['enemy02Texture'].texture
      });
    }
  ...
  public speedLimit = 350;

  public sightRangeSqr = Math.pow(1200, 2); //mikor lat meg
  public attackRangeSqr = Math.pow(200, 2); //mikor tamad
  public biteRangeSqr = Math.pow(80, 2); //mikor harap (sebez)
  public startRestDistanceSqr = Math.pow(100, 2); //mekkora távon pihen le a startpostól
